...
export async function estimateRemoveLiquidityPool(
    ...
    if (!market.hasWinner) {
        ...
        if (!results) return
        const minAmts = ammOutcomes.map((o, i) => ({
            amount: results[i] ?? '0',
            outcomeId: i,
    ...
    console.log(collateralOut, amm.totalLiquidity)
    const poolPct = formatPercentage(new BigNumber(collateralOut).dividedBy(amm.totalLiquidity))
